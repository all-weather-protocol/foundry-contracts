...
abstract contract AbstractVault is ERC4626, Ownable {
    ...
    function updateOneInchAggregatorAddress(address oneInchAggregatorAddress_) external onlyOwner {
    ...
    function totalStakedButWithoutLockedAssets() public view virtual returns (uint256);

    function totalUnstakedAssets() public view virtual returns (uint256);

    function totalAssets() public view override returns (uint256) {
        return totalLockedAssets() + totalStakedButWithoutLockedAssets() + totalUnstakedAssets();
    }
    ...
    function deposit(uint256 amount, bytes calldata oneInchData) public virtual returns (uint256) {
        _prepareForDeposit(amount);
    ...
    function deposit(
        ...
        _prepareForDeposit(amount);
        uint256 shares = _zapIn(WETH, amount, minLpOut, guessPtReceivedFromSy, input);
    ...
    function deposit(
        ...
        _prepareForDeposit(amount);
        uint256 shares = _zapIn(amount, oneInchData, minLpOut, guessPtReceivedFromSy, input);
        return _mintShares(shares, shares);
    }
    ...
    /* solhint-disable no-unused-vars */
    function _zapIn(uint256 amount, bytes calldata oneInchData) internal virtual returns (uint256) {
        revert("_zapIn not implemented");
    }
    ...
    function _mintShares(uint256 shares, uint256 amount) internal virtual returns (uint256) {
    ...
    /* solhint-disable no-unused-vars */
    function redeem(uint256 shares, IPendleRouter.TokenOutput calldata output) public virtual {
        revert("Not implemented");
    }
    ...
    function claimRewardsFromVaultToPortfolioVault(IFeeDistribution.RewardData[] memory claimableRewards)
        public
        virtual
    {
        for (uint256 i = 0; i < claimableRewards.length; i++) {
            SafeERC20.safeTransfer(IERC20(claimableRewards[i].token), msg.sender, claimableRewards[i].amount);
        }
    }
