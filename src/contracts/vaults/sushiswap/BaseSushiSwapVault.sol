...
contract BaseSushiSwapVault is AbstractVault {
    ...
    IERC20 public constant SUSHI_TOKEN = IERC20(0xd4d42F0b6DEF4CE0383636770eF773390d85c61A);
    ...
    IMiniChefV2 public constant sushiSwapMiniChef = IMiniChefV2(0xF4d73326C13a4Fc5FD7A064217e12780e9Bd62c3);
    ...
    function _initializeLpTokenAndRewarder(address token_consists_of_lp_, address rewarder_)
        internal
        onlyOnce("rewarder")
        onlyOwner
    {
    ...
    function _zapIn(uint256 amount, bytes calldata oneInchData) internal override returns (uint256) {
        uint256 wethAllowance = WETH.allowance(address(this), oneInchAggregatorAddress);
        if (wethAllowance > 0) {
            SafeERC20.forceApprove(WETH, oneInchAggregatorAddress, 0);
        }
        SafeERC20.forceApprove(WETH, oneInchAggregatorAddress, Math.mulDiv(amount, 1, 2));
        // slither-disable-next-line low-level-calls
        (bool succ, bytes memory data) = address(oneInchAggregatorAddress).call(oneInchData);
        require(succ, "1inch failed to swap, please update your block_number when running hardhat test");
        uint256 tokenReturnedAmount = abi.decode(data, (uint256));
        uint256 tokenAllowance = token_consists_of_lp.allowance(address(this), SUSHISWAP_ROUTER_ADDRESS);
        if (tokenAllowance > 0) {
            SafeERC20.forceApprove(token_consists_of_lp, SUSHISWAP_ROUTER_ADDRESS, 0);
        }
        SafeERC20.forceApprove(token_consists_of_lp, SUSHISWAP_ROUTER_ADDRESS, tokenReturnedAmount);
        ...
        (,, uint256 liquidity) = IUniswapV2Router01(SUSHISWAP_ROUTER_ADDRESS).addLiquidityETH{value: ethAmount}(
        ...
        uint256 slpAllowance = IERC20(asset()).allowance(address(this), address(sushiSwapMiniChef));
        if (slpAllowance > 0) {
            SafeERC20.forceApprove(IERC20(asset()), address(sushiSwapMiniChef), 0);
        }
        SafeERC20.forceApprove(IERC20(asset()), address(sushiSwapMiniChef), liquidity);
    ...
    function claim() public override {
        ...
        IFeeDistribution.RewardData[] memory claimableRewards = getClaimableRewards();
    ...
    function totalStakedButWithoutLockedAssets() public view override returns (uint256) {
    ...
    function getClaimableRewards() public view override returns (IFeeDistribution.RewardData[] memory rewards) {
        ...
        rewards[0] = IFeeDistribution.RewardData({
            ...
            amount: Math.mulDiv(
                sushiSwapMiniChef.pendingSushi(pid, address(this)), portfolioSharesInThisVault, totalVaultShares
                )
        });
        rewards[1] = IFeeDistribution.RewardData({
            token: address(token_consists_of_lp),
            amount: Math.mulDiv(rewarder.pendingToken(pid, address(this)), portfolioSharesInThisVault, totalVaultShares)
        });
        return rewards;
    }
