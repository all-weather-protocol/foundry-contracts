...
abstract contract BaseEquilibriaVault is AbstractVault {
    ...
    address public constant PENDLE_TOKEN_ADDR = 0x0c880f6761F1af8d9Aa9C466984b80DAb9a8c9e8;
    address public constant EQB_TOKEN_ADDR = 0xBfbCFe8873fE28Dfa25f1099282b088D52bbAD9C;
    address public constant XEQB_TOKEN_ADDR = 0x96C4A48Abdf781e9c931cfA92EC0167Ba219ad8E;
    ...
    function updatePendleBoosterAddr(address pendleBoosterAddr_) public onlyOwner {
    ...
    function _zapIn(
        ...
        uint256 currentAllowance = zapInToken.allowance(address(this), address(eqbZap));
    ...
    function redeem(uint256 shares) public override {
        // slither-disable-next-line unused-return
        (,, address rewardPool,) = pendleBooster.poolInfo(pid);
        uint256 currentAllowance = IBaseRewardPool(rewardPool).stakingToken().allowance(address(this), address(eqbZap));
        if (currentAllowance > 0) {
            SafeERC20.forceApprove(IBaseRewardPool(rewardPool).stakingToken(), address(eqbZap), 0);
        }
        SafeERC20.forceApprove(IBaseRewardPool(rewardPool).stakingToken(), address(eqbZap), shares);
    ...
    function getClaimableRewards() public view override returns (IFeeDistribution.RewardData[] memory rewards) {
        ...
        // slither-disable-next-line unused-return
        (,, address rewardPool,) = pendleBooster.poolInfo(pid);
        address[] memory rewardTokens = IBaseRewardPool(rewardPool).getRewardTokens();
        ...
        rewards[rewardTokens.length] = IFeeDistribution.RewardData({token: EQB_TOKEN_ADDR, amount: eqbAmount});
        return rewards;
    }
    ...
    function finalizeRedeem(uint256 redeemIndex) external onlyOwner {
        ...
        SafeERC20.safeTransfer(IERC20(EQB_TOKEN_ADDR), msg.sender, IERC20(EQB_TOKEN_ADDR).balanceOf(address(this)));
    }

    function _getEqbClaimableRewards(uint256 pendleAmount) internal view returns (uint256) {
