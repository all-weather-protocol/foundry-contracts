...
contract RadiantArbitrumVault is AbstractVault {
    ...
    constructor(address initialOwner, IERC20Metadata asset_, address radiantLending_)
        ERC4626(asset_)
    ...
    function totalLockedAssets() public view override returns (uint256) {
        ...
        (uint256 lockedBalances,,,,) =
            IFeeDistribution(0x76ba3eC5f5adBf1C58c91e86502232317EeA72dE).lockedBalances(address(this));
        return lockedBalances;
    }

    function totalStakedButWithoutLockedAssets() public pure override returns (uint256) {
        return 0;
    }
    ...
    function getRadiantRewardNativeTokenAddresses() external view returns (address[6] memory) {
    ...
    function redeem() public override {
        ...
        uint256 radiantDlpShares = MULTIFEE_DISTRIBUTION.withdrawExpiredLocksForWithOptions(address(this), 1, true);
    ...
    function getClaimableRewards() public view override returns (IFeeDistribution.RewardData[] memory rewards) {
        ...
        IFeeDistribution.RewardData[] memory radiantRewardData = MULTIFEE_DISTRIBUTION.claimableRewards(address(this));
        return _calculateClaimableRewards(radiantRewardData, portfolioSharesInThisVault, totalVaultShares);
    }
    ...
    function _calculateClaimableRewards(
        ...
        for (uint256 i = 0; i < radiantRewardData.length; i++) {
            ...
            radiantRewardData[i].amount =
                Math.mulDiv(radiantRewardData[i].amount, portfolioSharesInThisVault, totalVaultShares);
        }
